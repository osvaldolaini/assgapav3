            ...
            <fieldset class="grid grid-cols-12 gap-2 pb-6 rounded-md dark:bg-gray-900 items-start">
                <div class="col-span-6 grid grid-cols-12 gap-2">
                    ...
                    @if ($partner_id)
                        ...
                        <div class="col-span-12" >
                            <label for="location_date">*Datas</label>
                            <div class="w-full border p-2 rounded-md">
                                @isset($location_date)
                                    @foreach ($location_date as $date)
                                    @php
                                        $newDate = implode("-", array_reverse(explode("/", $date)));
                                        $newDate = strtotime($newDate);
                                    @endphp
                                    <div class="badge bg-red-400 gap-1 py-1 text-white cursor-pointer" wire:click='clean({{ $newDate }})'>
